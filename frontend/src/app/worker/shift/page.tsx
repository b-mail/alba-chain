...
import { AgentStepper } from "@/components/agents/agent-stepper";
...
const steps = [
  { id: "proof", label: "증빙 업로드" },
  { id: "vlm", label: "VLM 해석" },
  { id: "map", label: "온톨로지 매핑" },
  { id: "pay", label: "급여 그래프" },
];
...
export default function WorkerShiftAgentPage() {
  ...
  return (
    <AppShell
      role="worker"
      title="대타 · 추가근무"
      subtitle="개인톡 증빙 → VLM 해석 → 급여 자동 반영"
    >
      <Card className="mb-5 border-amber-200 bg-amber-50/60 p-4">
        <p className="text-sm text-amber-900">
          💬 카톡으로 받은 대타·연장 요청 스크린샷을 올리면, VLM이 맥락과
          시간을 해석해 <strong>근로계약 온톨로지</strong>에 매핑하고 실수령액
          예측을 업데이트합니다. 사장님께 따로 확인할 필요 없어요.
        </p>
      </Card>

      <AgentStepper steps={steps} currentStep={step} className="mb-6" />

      {step === 0 && !processing && (
        <Card className="mb-6">
          <CardHeader>
            <CardTitle>대타 근무 증빙 (개인톡)</CardTitle>
            <CardDescription>연장·대타 요청 대화 캡처</CardDescription>
          </CardHeader>
      ...
      {(step >= 1 || done) && (
        <>
          <Card className="mb-5">
            ...
            <dl className="grid gap-3 sm:grid-cols-2">
              {[
                ...
                { label: "매핑 계약", value: analysis.mappedContractNode },
              ].map(({ label, value }) => (
          ...
          <Card className="mb-5">
            <CardHeader>
              <CardTitle>온톨로지 매핑</CardTitle>
            </CardHeader>
            <OntologyChainView activeIndex={3} path={analysis.ontologyPath} />
          </Card>
            ...
            <>
              <Card className="mb-5">
                ...
                <p className="mt-4 text-center text-sm text-accent-600">
                  실수령 예상{" "}
                  <strong>{formatCurrency(886454)}</strong>
